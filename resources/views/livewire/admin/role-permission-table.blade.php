            ...
            <tbody>
                @foreach ($role->permissions as $permission)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-2">
                            <h2 class="font-bold text-gray-700">{{ $permission->created_at->toFormattedDateString() }}</h2>
                            <small class="text-gray-500">{{ $permission->created_at->diffForHumans() }}</small>
                        </td>
                        <td class="flex items-center px-6 py-2 space-x-3">
                        {{-- <a href="#" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a> --}}
                            <form action="{{ route('admin.role.permission.revoke', ['role' => $role->id, 'permission' => $permission->id]) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
