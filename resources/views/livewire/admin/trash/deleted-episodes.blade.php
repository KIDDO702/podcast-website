                ...
                @foreach ($episodes as $episode)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-2 py-1.5">
                            <h2 class="font-bold text-gray-700">{{ $episode->deleted_at->toFormattedDateString() }}</h2>
                            <small class="text-gray-500">{{ $episode->deleted_at->diffForHumans() }}</small>
                        </td>
                        <td class="flex items-center px-2 py-1.5 space-x-2">
                            <form wire:submit.prevent="restoreEpisode('{{ $episode->id }}')" method="post">
                                <button type="submit"
                            ...
                            <form wire:submit.prevent="deleteEpisode('{{ $episode->id }}')" method="post">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    onclick="return confirm('Are you sure you want to delete this episode?')"
