        ...
        <div class="container px-5 md:px-3 lg:px-0">
            <form action="{{ route('host.episode.update', ['show' => $show->slug, 'id' => $episode->id]) }}" method="POST" class="w-full" enctype="multipart/form-data">
                ...
                <div class="w-full">
                    ...
                    <div class="mt-16 lg:flex lg:items-start lg:space-x-10">
                        <div class="lg:w-[50%] bg-slate-700 p-7 rounded drop-shadow">
                        ...
                        <div class="mt-10 lg:mt-0 lg:w-[50%] bg-slate-700 p-7 rounded drop-shadow">
                            <div class="w-full">
                                <label for="youtube_link" class="font-semibold text-slate-200">YouTube Link</label>
                                <input type="text" name="youtube_link" id="youtube_link" value="{{ $episode->youtube_link }}" class="w-full mt-1 bg-slate-900 text-gray-300 px-4 py-2 rounded placeholder:text-gray-400 focus:outline focus:outline-slate-950 focus:outline-offset-2">
                                @error('youtube_link')
                                    <small class="font-semibold text-slate-200">{{ $message }}</small>
                                @enderror
                            </div>
                            <div class="w-full mt-7">
                                ...
                                <input type="text" name="spotify_link" id="spotify_link" value="{{ $episode->spotify_link }}" class="w-full mt-1 bg-slate-900 text-gray-300 px-4 py-2 rounded placeholder:text-gray-400 focus:outline focus:outline-slate-950 focus:outline-offset-2">
                                @error('spotify_link')
                                    <small class="font-semibold text-slate-200">{{ $message }}</small>
                                @enderror
                            </div>
            ...
            <form action="{{ route('host.episode.delete', $episode->id) }}" method="POST" class="mt-7">
                ...
                @csrf
                <button type="submit"
                    onclick="return confirm('Are you sure you want to delete {{ $episode->title }}?')"
