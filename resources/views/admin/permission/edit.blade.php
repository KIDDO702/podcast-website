...
@section('body')
    <div class="bg-white rounded drop-shadow-sm flex justify-between items-center py-2 px-4">
        <h3 class="font-bold text-slate-900 text-2xl">Permission</h3>
        ...
        <ul class="flex items-center space-x-3">
            ...
            <li>
                <a href="{{ route('admin.permission') }}" class="text-red-800 underline underline-offset-1">
                    <small class="font-semibold">Permission</small>
                </a>
            </li>
            <li>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3 flex items-center justify-center">
    ...
    <div class="mt-10 bg-white p-7 drop-shadow-sm rounded">
        <form action="{{ route('admin.permission.update', $permission->id) }}" method="POST">
            ...
            @csrf
            <div>
                <input type="text" value="{{ $permission->name }}" name="name" class="w-full bg-gray-50 border border-slate-200 rounded-sm px-4 py-2 focus:outline focus:outline-slate-400 focus:outline-offset-2">
                @error('name')
